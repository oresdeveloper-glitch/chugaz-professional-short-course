...
import {
  ...
  ChevronRight, Clock, CheckCircle2, AlertCircle, GraduationCap
} from "lucide-react"
          ...
          <nav className="flex-1 space-y-1">
            {sidebarItems.map((item) => (
              <button key={item.label} className={cn(
                ...
                item.active ? "bg-[#0B1F4D] text-white" : "text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800"
              )}>
          ...
          <div className="grid lg:grid-cols-3 gap-8">
                      ...
                      <CardContent className="p-4 md:p-6">
                        ...
                        <div className="flex items-center justify-between mt-4">
                          <span className="text-sm text-gray-500 flex items-center gap-1">
                            <Clock className="w-4 h-4" /> Duration: 2 Months
                          </span>
                        </div>
                      </CardContent>
                    </Card>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  { icon: Download, label: "Receipt", color: "text-green-600", bg: "bg-green-100" },
                  { icon: Award, label: "Certificate", color: "text-[#F4B400]", bg: "bg-yellow-100" },
                  { icon: User, label: "Profile", color: "text-blue-600", bg: "bg-blue-100" },
                  { icon: FileText, label: "Reports", color: "text-purple-600", bg: "bg-purple-100" },
                ].map((action) => (
                  <button key={action.label} className={`p-4 rounded-[20px] ${action.bg} hover:shadow-md transition-all text-center min-h-[64px]`}>
